...
#SBATCH -D ./ 
#SBATCH --job-name="sechr"
#SBATCH -o log-sepchr.out
#SBATCH -c 1
#SBATCH -p ibismini
...
java -cp $DIR SeparateChromosomes $file $sl $lod $mp $ap $t $tml $R $im $f > 03_output/map_"$base"_lod"$(echo "$lod"|sed 's/lodLimit=//g')"_sizeLimit"$(echo "$sl"|sed 's/sizeLimit=//g')".txt
